  ...
  try {
    ...
    if (typeOfAccount === ENUM_Account_Type.LOAN) {
      throw new ServerAPIError(
        ...
        "You can't send money from loan account"
      );
    }
    if (senderBalance < transaction.amount)
      throw new ServerAPIError(httpStatus.BAD_REQUEST, "Insufficient Balance");
    // const updateRefId = accountId.toString();
    const data = await TransactionAC.create(transaction);
    if (!data)
      throw new ServerAPIError(httpStatus.BAD_REQUEST, "Transaction failed");
    if (typeOfAccount === ENUM_Account_Type.CURRENT) {
      // set transactionRef in currentAC
      await CurrentAC.findByIdAndUpdate(accountId, {
        $push: {
    ...
    if (typeOfAccount === ENUM_Account_Type.SAVING) {
      await SavingAC.findByIdAndUpdate(accountId, {
        $push: {
          transactionRef: data._id,
        },
        $inc: {
    ...
    if (typeOfAccount === ENUM_Account_Type.STAFF) {
      await StaffAC.findByIdAndUpdate(accountId, {
        $push: {
          transactionRef: data._id,
        },
        $inc: {
    ...
    if (receiverTypeOfAccount === ENUM_Account_Type.CURRENT) {
      const account = await CurrentAC.findOne({
        accountNo: receiver?.accountNo,
      });
      await CurrentAC.findByIdAndUpdate(account?._id, {
        $push: {
          transactionRef: data._id,
        },
        $inc: {
    ...
    if (receiverTypeOfAccount === ENUM_Account_Type.SAVING) {
      const account = await SavingAC.findOne({
        accountNo: receiver?.accountNo,
      });
      await SavingAC.findByIdAndUpdate(account?._id, {
        $push: {
          transactionRef: data._id,
        },
        $inc: {
    ...
    if (receiverTypeOfAccount === ENUM_Account_Type.STAFF) {
      ...
      await StaffAC.findByIdAndUpdate(account?._id, {
        $push: {
          transactionRef: data._id,
        },
        $inc: {
...
const getAllTransaction = async () => {
  const data = await TransactionAC.find()
    .populate("userId")
    .populate("receiverId");
  return data;
};
...
const getTransactionById = async (id: string) => {
  const data = await TransactionAC.findById(id)
    .populate("userId")
    .populate("receiverId");
  if (!data)
    throw new ServerAPIError(httpStatus.NOT_FOUND, "Transaction not found");
  return data;
};

// my transaction
const myTransaction = async (userId: string) => {
  const data = await TransactionAC.find({ userId })
    .populate("userId")
    .populate("receiverId");
  if (!data)
    throw new ServerAPIError(httpStatus.NOT_FOUND, "Transaction not found");
